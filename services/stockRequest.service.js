...
const updateStockRequestById = async (id, data) => {
  ...
  const updatedStockRequest = await StockRequest.findByIdAndUpdate(id, data, {
    new: true,
  });
  ...
  if (data.status === "Approved") {
    ...
    // Step 4: Find the outlet and dispatch associated with the stock request
    const outlet = await Outlet.findById(stockRequest.outletId);
    const dispatch = await Dispatch.findById(stockRequest.dispatchId);

    if (!outlet) throw new Error("Outlet not found while updating stock.");
    if (!dispatch) throw new Error("Dispatch not found while reducing stock.");

    // Step 5: Update the outlet's gas stock (Adding the delivered stock)
    stockRequest.stockDetails.forEach((item) => {
      ...
      if (stockIndex !== -1) {
        // Update existing gas stock quantity in Outlet
        outlet.gasStock[stockIndex].quantity += item.quantity;
      } else {
        // Add new gas type if it doesn't exist in Outlet
    ...
    // Step 6: Reduce the quantity from Dispatch gas stock
    stockRequest.stockDetails.forEach((item) => {
      const dispatchStockIndex = dispatch.gasStock.findIndex(
        (gs) =>
          gs.gasType === item.gasType &&
          gs.weight === item.weight &&
          gs.individualPrice === item.individualPrice
      ...
      if (dispatchStockIndex !== -1) {
        // Reduce the quantity in Dispatch
        if (dispatch.gasStock[dispatchStockIndex].quantity >= item.quantity) {
          dispatch.gasStock[dispatchStockIndex].quantity -= item.quantity;
        } else {
          throw new Error(
            `Insufficient stock in dispatch for ${item.gasType} - ${item.weight} KG`
          );
        }
      } else {
        throw new Error(
          `Dispatch stock not found for ${item.gasType} - ${item.weight} KG`
        );
      }
    });

    // Step 7: Save the updated outlet and dispatch stock
    updatedOutlet = await outlet.save();
    updatedDispatch = await dispatch.save();
  }

  // Step 8: Send notification if there's a message
  ...
  // Step 9: Return the updated stock request, outlet, and dispatch
  return { updatedStockRequest, updatedOutlet, updatedDispatch };
};
